...
import { PaperTrackDemo, type TrackStatus, type TrackQuality } from '@/ar/PaperTrackDemo';
...
export function CameraARView({
  ...
  const [arQuality, setArQuality] = useState<TrackQuality | null>(null);
  ...
  const processCanvas = useCallback(
    async (sourceCanvas: HTMLCanvasElement, thresh: number, expand: number, forceRespawn: boolean) => {
      ...
      onDebugUpdate({ gridResolution: `${GRID_WIDTH}x${GRID_HEIGHT}`, solidPixels: solidCount, threshold: thresh });
  ...
  const enterArTest = useCallback(async () => {
    ...
    demo.setStatusHandler((s, detail, quality) => {
      setArStatus(s);
      setArDetail(detail ?? '');
      if (quality !== undefined) setArQuality(quality ?? null);
    });
    try {
  ...
  const arBadge = (() => {
    ...
    if (arStatus === 'weak_target') return { text: 'Weak target', color: 'bg-red-600/90' };
    if (arStatus === 'loading') return { text: arDetail || 'Loading AR…', color: 'bg-amber-600/90' };
    if (arStatus === 'compiling') return { text: arDetail || 'Compiling…', color: 'bg-amber-600/90' };
    if (arStatus === 'searching') return { text: 'Searching…', color: 'bg-amber-600/90' };
    if (arStatus === 'lost') return { text: 'Paper lost', color: 'bg-amber-700/90' };
    if (arStatus === 'error') return { text: 'AR error', color: 'bg-red-600/90' };
      ...
      <div ref={viewportRef} className="relative flex-1 overflow-hidden">
              ...
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none px-6 py-8">
                ...
                >
                  <span className="absolute -bottom-6 left-0 right-0 text-center text-[10px] font-hand text-paper-100/70">
                    Align letter paper (8.5x11) inside frame
                  </span>
                </div>
              </div>
            )}
          ...
          <div className="flex-1 flex justify-center">
            {arTestMode ? (
              <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-paper-100 text-xs font-hand font-bold ${arBadge.color}`}>
                {(arStatus === 'compiling' || arStatus === 'searching' || arStatus === 'loading') && (
                  <Loader2 size={14} className="animate-spin" />
                )}
                {arStatus === 'found' && <CheckCircle2 size={14} />}
                {(arStatus === 'error' || arStatus === 'weak_target') && <XCircle size={14} />}
                {arBadge.text}
              </div>
            ) : (
        ...
        {arTestMode && (
          <div className="absolute bottom-4 left-3 right-3 z-20">
            <div
              className={`rounded-xl border-2 px-3 py-3 text-left shadow-lg ${
                arStatus === 'found'
                  ? 'bg-green-100/95 border-green-700 text-ink-900'
                  : arStatus === 'error' || arQuality?.level === 'poor'
                    ? 'bg-red-100/95 border-red-600 text-ink-900'
                    : arQuality?.level === 'weak' || arStatus === 'weak_target'
                      ? 'bg-amber-100/95 border-amber-600 text-ink-900'
                      : 'bg-paper-200/95 border-ink-800/30 text-ink-900'
              }`}
            >
              <p className="text-sm font-hand font-bold">
                {arStatus === 'found'
                  ? 'Tracking OK — spinning cube is on the paper'
                  : arDetail || arBadge.text}
              </p>
              {arQuality && (
                <p className="text-xs font-hand mt-1">
                  Features detected: <span className="font-bold">{arQuality.featureCount}</span>
                  {' · '}
                  {arQuality.level === 'good' && 'Good detail'}
                  {arQuality.level === 'weak' && 'Weak detail'}
                  {arQuality.level === 'poor' && 'Not enough detail'}
                </p>
              )}
              {(arQuality?.level === 'weak' || arQuality?.level === 'poor' || arStatus === 'error') && (
                <ul className="mt-2 text-[11px] font-hand list-disc list-inside space-y-0.5 opacity-90">
                  {(arQuality?.tips ?? [
                    'Draw denser lines or X marks in the corners',
                    'Retake a sharper photo',
                    'Or try a printed high-contrast image',
                  ]).map((tip) => (
                    <li key={tip}>{tip}</li>
                  ))}
                </ul>
              )}
              {arStatus === 'searching' && arQuality?.level === 'good' && (
                <p className="text-[11px] font-hand mt-2 opacity-80">
                  Hold the same drawing in view. Cube + green ring appear when locked.
                </p>
              )}
              {(arStatus === 'loading' || arStatus === 'compiling') && (
                <p className="text-[11px] font-hand mt-2 opacity-80">Please wait…</p>
              )}
            </div>
          </div>
        )}
        ...
        {showDebug && !arTestMode && (
          <div className="absolute top-14 left-3 right-3 max-w-xs z-20">
            <div className="bg-paper-300/70 backdrop-blur-md border-2 border-ink-800/20 rounded-lg p-3 space-y-2 text-xs font-hand">
              ...
              <div className="flex justify-between text-ink-700"><span>Capture:</span><span className="font-bold">{CAPTURE_WIDTH}x{CAPTURE_HEIGHT}</span></div>
              <div className="flex justify-between text-ink-700"><span>Grid:</span><span className="font-bold">{GRID_WIDTH}x{GRID_HEIGHT}</span></div>
            </div>
          </div>
        )}
        ...
        {showFilters && hasGrid && !arTestMode && (
          <div className="absolute bottom-4 left-3 right-3 z-20">
            <div className="bg-paper-200/95 backdrop-blur-md border-2 border-ink-800/25 rounded-xl p-4 space-y-3 shadow-lg">
              ...
              <div className="space-y-1">
                <div className="flex justify-between"><label className="text-sm font-hand font-bold text-ink-800">Player size</label><span className="text-sm font-hand font-bold">{playerScale.toFixed(1)}x</span></div>
              ...
              <div className="space-y-1">
                <div className="flex justify-between"><label className="text-sm font-hand font-bold text-ink-800">World scale</label><span className="text-sm font-hand font-bold">{worldScale.toFixed(1)}x</span></div>
